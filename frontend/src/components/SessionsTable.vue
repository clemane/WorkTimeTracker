...
async function sendReportByEmail(startMonday: string) {
  const rawUser = window.localStorage.getItem("worktime:user");
  const user = rawUser ? JSON.parse(rawUser) as { id: number } : null;
  if (!user?.id) return;

  sendingEmail.value = startMonday;
  emailSuccess.value = null;

  try {
    const url = `/api/report?monday=${encodeURIComponent(startMonday)}&userId=${encodeURIComponent(String(user.id))}&mode=${viewMode.value}&format=pdf&sendEmail=true`;
    const res = await fetch(url);
    if (!res.ok) throw new Error("Erreur lors de l'envoi");

    emailSuccess.value = startMonday;
    setTimeout(() => { emailSuccess.value = null; }, 3000);
  } catch (e) {
    alert("Erreur lors de l'envoi du rapport par courriel.");
  } finally {
      ...
      >
        ...
        <div class="flex items-center justify-between sm:justify-end gap-6">
          ...
          <div class="flex items-center gap-2">
            ...
            <button 
              @click="sendReportByEmail(g.startMonday)"
              :disabled="!!sendingEmail"
              class="h-10 w-10 flex items-center justify-center rounded-xl border border-border hover:bg-surface-hover text-text-muted hover:text-text-body transition-all disabled:opacity-50"
              :title="emailSuccess === g.startMonday ? 'Envoyé !' : 'Envoyer par courriel à Fred'"
            >
              <Loader2 v-if="sendingEmail === g.startMonday" class="h-4 w-4 animate-spin text-primary" />
              <CheckCircle2 v-else-if="emailSuccess === g.startMonday" class="h-4 w-4 text-green-400" />
