...
async function sendReportByEmail() {
  ...
  try {
    const url = `/api/report?monday=${encodeURIComponent(monday)}&userId=${encodeURIComponent(String(user.id))}&mode=${mode.value}&format=pdf&sendEmail=true`;
    ...
    <div class="flex justify-center pt-8 pb-12">
      <div class="bg-surface border border-border p-2 rounded-3xl shadow-xl flex items-center gap-2">
        ...
        <button 
          ...
          class="flex items-center gap-2 px-4 py-3 text-sm font-bold text-text-muted hover:text-text-body hover:bg-surface-hover rounded-2xl transition-all disabled:opacity-50"
          :title="emailSuccess ? 'Envoyé !' : 'Envoyer par courriel à Fred'"
        >
          <Loader2 v-if="sendingEmail" class="h-4 w-4 animate-spin text-primary" />
          <Mail v-else-if="!emailSuccess" class="h-4 w-4" />
          <CheckCircle2 v-else class="h-4 w-4 text-green-400" />
          <span class="hidden sm:inline">{{ sendingEmail ? 'Envoi...' : emailSuccess ? 'Envoyé' : 'Envoyer' }}</span>
        </button>
        <button 
          @click="saveWeek"
          :disabled="saving"
